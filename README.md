...
public:
	...
	//vector(vector&& b) : x(b.x), y(b.y), z(b.z) {} //rvalue?ссылка на временный объект.
	...
	const vector operator++(int); // int если постфиксный декремент 
	vector operator++();
...
const vector vector::operator++(int) { //постфиксный, поэтому инт в скобках
	vector c= *this;
	this->x += 1;
	this->y += 1;
	this->z += 1;
...
vector vector::operator*(double i) { // const double& i - нельзя изменять объект
	vector c;
	c.x = x * i;
	c.y = y * i;
	c.z = z * i;
...
vector vector::rotate(vector& b, double fi) {
	...
	double fi_r = pi * fi / 180;

	vector c = b.unit(); // бэкаю вектор коллинеарный b 

	double cos_fi_r = cos(fi_r), sin_fi_r = sin(fi_r), help = 1.0 - cos_fi_r; // вспомогательные для формулы Родрига

	double scalar = c & (*this); // скалярное произведение

	vector vectornoe_proiz = c ^ (*this); //векторное

	return (*this) * cos_fi_r + vectornoe_proiz * sin_fi_r + c * (scalar * help); //формула Родрига
}
...
int main()
{
    ...
    //(a&b)=2.0; //результат число и числу присвоить число странно...
    std::cout << a << " " << b << std::endl;
    ...
    b.set(3.0, 1.5, 2.0);
    alpha = 30.0;
    std::cout << a.rotate(b, alpha) << " " << a << " " << b << " " << std::endl;
}
